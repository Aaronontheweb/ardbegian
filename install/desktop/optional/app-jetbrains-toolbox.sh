...
if ! tar -xzf jetbrains-toolbox.tar.gz -C "$TOOLBOX_TEMP_DIR"; then
...
TOOLBOX_DIR=$(find "$TOOLBOX_TEMP_DIR" -maxdepth 1 -type d -name "jetbrains-toolbox-*" | head -1)
if [ -z "$TOOLBOX_DIR" ]; then
    echo "Error: Could not find JetBrains Toolbox directory in extracted files"
...
# Find the binary
TOOLBOX_BINARY=$(find "$TOOLBOX_DIR" -name "jetbrains-toolbox" -type f -executable | head -1)
if [ -z "$TOOLBOX_BINARY" ]; then
    echo "Error: Could not find JetBrains Toolbox binary"
    echo "Contents of toolbox directory:"
    ls -la "$TOOLBOX_DIR"
    exit 1
fi

echo "Found binary: $TOOLBOX_BINARY"

# Copy the entire toolbox directory to the installation location
INSTALL_DIR="$HOME/.local/share/JetBrains/Toolbox/$(basename "$TOOLBOX_DIR")"
if ! cp -r "$TOOLBOX_DIR" "$INSTALL_DIR"; then
    echo "Error: Failed to copy JetBrains Toolbox to installation directory"
    exit 1
fi

# Make sure the binary is executable
chmod +x "$INSTALL_DIR/bin/jetbrains-toolbox"
TOOLBOX_BINARY="$INSTALL_DIR/bin/jetbrains-toolbox"
